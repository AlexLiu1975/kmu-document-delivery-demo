...
function adapter(ipFails=false, staffAllowed=true, authDelay=0){
 ...
 const window={FirebaseStoreCore:require('../firebase-store-core'),UsageCore:require('../usage-core'),DocumentStatus:require('../status'),dispatchEvent:()=>{}};
 const context={window,crypto:require('node:crypto').webcrypto,URL,AbortController,setTimeout,clearTimeout,CustomEvent:class{constructor(type,options){this.type=type;this.detail=options?.detail;}},localStorage:{getItem:k=>storage.get(k),setItem:(k,v)=>storage.set(k,v),removeItem:k=>storage.delete(k)},sessionStorage:{getItem:k=>storage.get(k),setItem:(k,v)=>storage.set(k,v),removeItem:k=>storage.delete(k)},initializeApp:()=>({}),initializeAuth:()=>auth,browserSessionPersistence:{},signInWithEmailAndPassword:async(a,email,password)=>{if(password!=='fixture-password-123')throw Error('invalid');auth.currentUser={uid:'staff-a',isAnonymous:false};},getDoc:async ref=>ref.path.startsWith('staffAdmins/')
   ? {exists:()=>staffAllowed&&!auth.currentUser?.isAnonymous,data:()=>({enabled:true,employeeNumber:'1115034'})}
   : {exists:()=>ref.path.endsWith('1151100099'),data:()=>({documentNumber:'1151100099',year:'115',typeCode:'11',serial:'00099',status:'R',assignee:'1115034'})},updatePassword:async(user,password)=>{assert.equal(user,auth.currentUser);assert.equal(user.uid,'staff-a');assert.equal(typeof password,'string');},signInAnonymously:async()=>{if(authDelay)await new Promise(r=>setTimeout(r,authDelay));auth.currentUser={uid:'uid-a',isAnonymous:true};},signOut:async()=>{auth.currentUser=null;},getFirestore:()=>({}),firebaseConfig:{},collection:()=>({}),doc:(db,...parts)=>({path:parts.join('/')}),serverTimestamp:()=> 'TIME',addDoc:async(ref,record)=>writes.push(record),runTransaction:async(db,callback)=>{await callback({get:async()=>({exists:()=>false}),set:()=>{}});},fetch:async()=>{if(ipFails)throw Error('offline');return {ok:true,json:async()=>({ip:'203.0.113.7'})};}};
...
test('general employee reads only one exact document through queryDocument',async()=>{const {store}=adapter();await settle();await store.login('1234567');const found=await store.queryDocument('1151100099');assert.equal(found.documentNumber,'1151100099');assert.equal(await store.queryDocument('1151100098'),null);await assert.rejects(()=>store.queryDocument('bad-number'));});
